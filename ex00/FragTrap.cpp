...
/*   Updated: 2021/10/08 23:56:54 by tayamamo         ###   ########.fr       */
/*   Copyright 2021                                                           */
/* ************************************************************************** */
...
FragTrap::~FragTrap() { cout(": Good bye! God bless you!"); }
...
unsigned int FragTrap::getHitPoints() const { return (this->hit_points_); }
...
unsigned int FragTrap::getLevel() const { return (this->level_); }
...
std::string FragTrap::getType() const { return (this->type_); }

void FragTrap::setHitPoints(unsigned int amount) { this->hit_points_ = amount; }
...
void FragTrap::rangeAttack(std::string const& target) {
    if (this->getHitPoints() > 0) {
        cout() << "attacks " << target << " at range, causing "
               << this->getRangedAttackDamege() << " points of damage!"
               << std::endl;
    } else {
    ...
    if (this->getHitPoints() == this->getMaxHitPoints() &&
        ...
        if (this->getHitPoints() != this->getMaxHitPoints()) {
            ...
            cout() << "has regained " << this->getHitPoints() - tmp
                   ...
                   << "." << std::endl;
            if (tmp == 0) cout() << "can be fight again. YEAAAAH!" << std::endl;
        } else {
            cout() << "has " << this->getHitPoints() << " hit points."
                   << std::endl;
        }
        ...
        if (this->getEnergyPoints() != this->getMaxEnergyPoints()) {
            ...
            if (this->getHitPoints() > 0 && tmp < 25 &&
                this->getEnergyPoints() >= 25)
                cout() << "can be special attack again. YEAAAAH!" << std::endl;
        } else {
            cout() << "has " << this->getEnergyPoints() << " energy points."
                   << std::endl;
        }
    }
}
...
void FragTrap::vaulthunter_dot_exe(std::string const& target) {
    static const std::string magics[] = {"fire", "ice", "thunder", "wind",
                                         "water"};
    static const unsigned int damages[] = {1, 2, 3, 4, 5};
    ...
    if (this->getHitPoints() > 0) {
        if (this->getEnergyPoints() >= 25) {
            ...
            cout() << "attacks " << target << " at " << magic << ", causing "
                   << damage << " points of damage! Energy points is "
